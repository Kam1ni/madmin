	...
	<v-app>
		<template v-if="loggedIn">
			<v-navigation-drawer app clipped fixed v-model="drawer">
				<v-list>
					<v-list-tile to="/" exact active-class="primary white--text">
						<v-list-tile-content>
							Home
						</v-list-tile-content>
					</v-list-tile>
					...
					<v-list-tile to="/statics" active-class="primary white--text">
						<v-list-tile-content>
							Statics
						</v-list-tile-content>
					</v-list-tile>
			...
			<v-toolbar app fixed clipped-left class="primary" dark>
        		...
        		<span class="hidden-xs-only">Madmin</span>
			</v-toolbar>
		...
		<v-layout row justify-center wrap align-center class="primary" fill-height v-if="!loggedIn">
			<v-flex xs10 sm8 md6>
				<app-login></app-login>
			</v-flex>
		</v-layout>
